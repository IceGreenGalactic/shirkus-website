...
function getISOWeek(date) {
  const key = getISOWeekKey(ymdOslo(date));
...
function getLegacyStats(title, start) {
  if (title === "Dag") {
    const day = legacyStats.global?.days?.[start];

    return {
      sessions: Number(day?.sessions || 0),
      uniques: Number(day?.uniqueVisitors || 0),
    };
  }
  ...
  if (title === "Uke") {
    ...
    const week = legacyStats.global?.weeks?.[key];

    return {
      sessions: Number(week?.sessions || 0),
      uniques: Number(week?.uniqueVisitors || 0),
    };
  }
  ...
  if (title === "Måned") {
    ...
    const month = legacyStats.global?.months?.[key];

    return {
      sessions: Number(month?.sessions || 0),
      uniques: Number(month?.uniqueVisitors || 0),
    };
  }
  ...
  if (title === "År") {
    ...
    const year = legacyStats.global?.years?.[key];

    return {
      sessions: Number(year?.sessions || 0),
      uniques: Number(year?.uniqueVisitors || 0),
    };
  }
...
async function getPosthogRange(start, end) {
  ...
  if (!response.ok) {
    throw new Error("Kunne ikke hente PostHog range-statistikk");
  }
  ...
  return {
    ...
    uniques: Number(json.totals?.uniqueVisitors || 0),
  };
}

export default function RangeCard({ title, items, refreshKey = 0 }) {
  ...
  const smartLabel = useMemo(() => {
    ...
    if (title === "Dag") {
      if (start === todayYmd && end === todayYmd) {
        return "I dag";
      }

      if (start === yesterdayYmd && end === yesterdayYmd) {
        return "I går";
      }
    ...
    if (title === "Uke") {
      ...
      return `Uke ${getISOWeek(startDate)}`;
    }
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
         ...
         */
        if (start <= LEGACY_END) {
          const legacy = getLegacyStats(title, start);

          sessions += legacy.sessions;

          uniques += legacy.uniques;
        }
        ...
        if (end >= POSTHOG_START) {
          const posthogStart = start >= POSTHOG_START ? start : POSTHOG_START;

          const posthog = await getPosthogRange(posthogStart, end);

          sessions += posthog.sessions;
        ...
        if (!cancelled) {
          setData({
            ...
            uniquesGlobal: uniques,
          });
        }
      } catch (error) {
        console.error("Kunne ikke hente range-statistikk:", error);
      } finally {
        if (!cancelled) {
          setIsLoading(false);
        }
  ...
  }, [cur.start, cur.end, title, refreshKey]);
  ...
  return (
    <RangeCardWrap>
      <RangeTop>
        <BtnIcon onClick={next} aria-label="Forrige" disabled={isLoading}>
          ←
        </BtnIcon>

        <RangeTitleCenter>{title}</RangeTitleCenter>

        <BtnIcon onClick={prev} aria-label="Neste" disabled={isLoading}>
          →
        </BtnIcon>
      </RangeTop>
      ...
      <RangeMetrics>
        <Metric>
          ...
          <MetricValue>
            {isLoading ? "…" : nf.format(data.sessionsTotal)}
          </MetricValue>
        </Metric>
        ...
        <Metric>
          ...
          <MetricValue>
            {isLoading ? "…" : nf.format(data.uniquesGlobal)}
          </MetricValue>
        </Metric>
      </RangeMetrics>
    </RangeCardWrap>
