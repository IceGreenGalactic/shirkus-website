    ...
    sanityClient
      .fetch(
        `*[_type == "litter" && _id == $id]{
          ...
          additionalImages,
          textUnderImages,
          dateOfBirth // Make sure to fetch dateOfBirth too
        }`,
        { id }
      )
      .then((data) => {
    ...
    <LitterContainer className="container">
      ...
      <div className="row">
        ...
        <div className="col-md-6">
          <ParentInfo>
            <h3>Mor: {litter.mother.nickname} ({litter.mother.name})</h3>
            {litter.mother.imageUrl && (
              <ParentImage src={litter.mother.imageUrl} alt={litter.mother.name} />
            )}
        ...
        <div className="col-md-6">
          <ParentInfo>
            <h3>Far: {litter.father.nickname} ({litter.father.name})</h3>
            {litter.father.imageUrl && (
              <ParentImage src={litter.father.imageUrl} alt={litter.father.name} />
            )}
      ...
      {/* Date of Birth and Main Image */}
      {litter.dateOfBirth && (
        <h4>Dato født: {new Date(litter.dateOfBirth).toLocaleDateString()}</h4>
      )}
      {litter.mainImage && <img src={litter.mainImage.asset.url} alt="Kull bilde" />}

      {/* Puppy Details */}
      <h4>Detaljer om valper</h4>
      {litter.puppyDetails && litter.puppyDetails.map((puppy, index) => (
        <div key={index}>
          <h5>Valp {index + 1}:</h5>
      ...
      {litter.additionalImages && litter.additionalImages.length > 0 && (
        <PuppyGallery>
          ...
          {litter.additionalImages.map((image, index) => (
            <PuppyImage key={index} src={image.asset.url} alt={`Valp bilde ${index + 1}`} />
          ))}
        </PuppyGallery>
      )}
